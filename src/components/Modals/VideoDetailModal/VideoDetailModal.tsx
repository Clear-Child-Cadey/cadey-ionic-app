...
const VideoDetailModal: React.FC = () => {
  ...
  const { requestQuiz } = useRequestQuiz({
    clientContext: 1,
    entityType: 1,
    entityId: Number(videoData?.mediaId),
  });
  ...
  useEffect(() => {
    // const isMounted = true; // To avoid state updates on unmounted component
                  ...
                  {relatedMedia.relatedMediaItems.map((item, itemIndex) => (
                    <div className='related-item' key={itemIndex}>
                      ...
                      {item.mediaType === 1 && item.sourceId && (
                        <div
                          onClick={() =>
                            handleRelatedVideoClick(item.sourceId!)
                          }
