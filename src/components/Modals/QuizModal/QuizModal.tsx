...
const QuizModal: React.FC = ({}) => {
  ...
  const { isQuizModalOpen, setQuizModalOpen, quizModalData, setQuizModalData } =
    useModalContext();
  ...
  const [textResponses, setTextResponses] = React.useState<{
    [key: number]: string;
  }>({}); // Key = optionId, Value = text response
    ...
    if (userResponse.includes(response)) {
      ...
      setSelectedOptionIds(
        selectedOptionIds.filter(
          (item) =>
            item !==
            quizModalData!.question.options.find(
              (option) => option.label === response,
            )!.id,
        ),
      );
  ...
  const handleSubmission = async () => {
    ...
    const updatedResponses = quizModalData!.question.options
      .map((option) => {
    ...
    updatedResponses.forEach((newResponse) => {
      const existingIndex = mergedResponses.findIndex(
        (r) => r.optionId === newResponse!.optionId,
      );
      if (existingIndex > -1) {
  ...
  const sendQuizResponse = async (
    ...
    try {
      ...
      if (
        quizSubmissionResponse.quizRequestResponseModel.question !== null &&
        quizSubmissionResponse.quizRequestResponseModel.question !==
          undefined &&
        quizSubmissionResponse.quizRequestResponseModel.question.id !== 0
      ) {
  ...
  function handleClose() {
    ...
    dispatch(setQuizModalOpenRx(false));
  }
          ...
          <IonRow className='quiz-content'>
            ...
            <IonRow className='explanation'>
              <IonText className='explanation-text'>
                {quizModalData.question.introMessage}
              </IonText>
            </IonRow>
            <IonRow className='question'>
              <IonText className='question-text'>
                {quizModalData.question.text}
              </IonText>
            </IonRow>
            ...
            <IonRow className='responses'>
              {quizModalData.question.options.map((option, index) => {
                ...
                if (option.optionType === 1) {
                  ...
                  option.optionType === 2 &&
                  option.displayIfOptionIdSelected === null
                ) {
                  ...
                  return (
                    <IonRow key={index} className='text-input-row'>
                      ...
                      <textarea
                        ...
                        onBlur={(e) =>
                          (e.target.placeholder = 'Type your response here')
                        }
                        onChange={(e) =>
                  ...
                  const displayIfOptionIds = option.displayIfOptionIdSelected
                    .split(',')
                    .map((id) => Number(id));
                  if (
                    displayIfOptionIds.some((id) =>
            ...
            <IonRow className='continue-row'>
              ...
              >
                {quizModalData.nextQuestionPossible ? <>Next</> : 'Finish'}
              </IonButton>
            </IonRow>
          </IonRow>
        </IonContent>
